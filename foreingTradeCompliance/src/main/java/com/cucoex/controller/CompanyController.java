...
public class CompanyController {
	...
	private static final org.slf4j.Logger log = LoggerFactory.getLogger(CompanyController.class);
	...
	public CompanyController() {
			  ...
			  if(result.hasErrors()) { 
				    ...
				    try {
						...
						try {
							...
							log.info("Registros insertados " + complianListAdded.size());
							log.info("Registros eliminado " + complianListRemoved.size());
		...
		public String getEditCompanyForm(Model model, @PathVariable(name ="id")Long id)throws Exception{
			...
			log.info("Entrando a getEditCompanyForm con Id " + id);
		...
		public String cancelEditCompany(ModelMap model) {
			log.info("Entrando a companyFormCancel");
		...
		public String deleteUser(Model model, @PathVariable(name="id")Long id) {
			try {
				log.info("Borrando empresa " +id);
				companyService.deleteCompany(id);
			
			} 
			catch (CompanyException uoin) {
				log.error("Excepcion al borrar empresa " +id);
